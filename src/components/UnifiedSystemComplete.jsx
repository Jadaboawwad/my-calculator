    ...
    try {
      ...
      if (navigator.geolocation) {
        try {
          ...
          try {
            const geoResponse = await fetch(`https://api.aladhan.com/v1/timings/${day}-${month}-${year}?latitude=${latitude}&longitude=${longitude}&method=4`);
 ...
 const getRecommendations = (h, m, power, reasons) => {
  ...
  const quranicRecommendations = {
    ...
    1: {
      type: 'عبادة روحانية',
      actions: [
        '🤲 دعاء التوحيد والإخلاص',
        '📖 قراءة سورة الإخلاص',
        '🕌 صلاة نافلة بخشوع',
        '💭 التفكر في وحدانية الله',
        '🎯 التركيز على هدف واحد مهم'
      ]
    },
    // رقم 2 - التوازن
    2: {
      type: 'توازن وتدبر',
      actions: [
        '⚖️ إعادة التوازن بين الأعمال',
        '🤲 التوكل على الله',
        '📖 قراءة قرآن بتدبر',
        '📝 مراجعة الأولويات',
        '💼 عمل متوازن بين دنيا وآخرة'
      ]
    },
    // رقم 3 - الصبر (Tesla)
    3: {
    ...
    6: {
      ...
      actions: [
        ...
        '💭 التفكر في حكمة التوقيت',
        '📝 تأجيل القرارات المستعجلة',
        '🎯 التخطيط طويل المدى'
      ]
    },
    // رقم 7 - البركة ⭐
    ...
    9: {
      type: 'حذر وتوبة',
      actions: [
        '⚠️ الابتعاد عن المعاصي',
        '🤲 الاستعاذة بالله من الشر',
        '📝 مراجعة الأعمال والنوايا',
        '💚 التوبة والاستغفار',
        '✨ عمل صالح يمحو السيئات'
      ]
    },
    // رقم 11 - البشارة
    11: {
      type: 'بشائر وأمل',
      actions: [
        '⭐ الانتباه للإشارات الإيجابية',
        '💭 التفكر في البشائر',
        '🤲 دعاء تحقيق الأحلام',
        '📝 تخطيط مستقبلي واعد',
        '🎯 كتابة الأهداف والطموحات'
      ]
    },
    // رقم 12 - التنظيم
    12: {
  ...
  const hReduced = h > 9 ? String(h).split('').reduce((a, b) => parseInt(a) + parseInt(b), 0) : h;
  const mReduced = m > 9 ? String(m).split('').reduce((a, b) => parseInt(a) + parseInt(b), 0) : m;
            ...
            {tesla369Times.length > 0 && (
              <div className="bg-gradient-to-br from-yellow-900/30 via-orange-900/30 to-red-900/30 backdrop-blur-lg rounded-2xl p-4 md:p-6 border-2 border-yellow-400/50 shadow-lg shadow-yellow-500/20">
                ...
                <div className="mb-4 p-4 bg-yellow-500/20 rounded-lg border border-yellow-400/50">
                  <div className="text-sm text-yellow-200 text-center mb-3">🌟 الأوقات التي تحتوي 3 و 6 و 9 معاً - أقوى أوقات اليوم!</div>
                  {tesla369Times[0] && (
                    <div className="p-4 bg-gradient-to-r from-yellow-600/30 to-orange-600/30 rounded border border-yellow-300">
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="text-4xl font-bold text-yellow-100">{formatTime(tesla369Times[0].hour, tesla369Times[0].minute)}</div>
                        </div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  ...
                  {tesla369Times.slice(1, 8).map((time, i) => (
                    <div key={i} className="p-3 rounded bg-gradient-to-r from-yellow-800/20 to-orange-800/20 border border-yellow-500/30">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <span className="text-yellow-400 font-bold text-lg">{formatTime(time.hour, time.minute)}</span>
            ...
            <div className="bg-gradient-to-r from-green-900/30 to-emerald-900/30 backdrop-blur-lg rounded-2xl p-4 md:p-6 border-2 border-green-400/50">
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {quranNumbers.basics &&
                    quranNumbers.basics.map((item, i) => (
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {quranNumbers.surahs &&
                    quranNumbers.surahs.map((item, i) => (
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {quranNumbers.wordsBalance &&
                    quranNumbers.wordsBalance.map((item, i) => (
